...
**Secrets:** no plaintext committed. `toastify0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret toastify0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `toastify0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `toastify0` | `sdk-apps-dev` | toastify0-dev-topia.topia-rtsdk.com | `/api/system/health` |
